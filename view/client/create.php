<div class="container">
	<form action="<?= URL ?>/client/createSave" method="post">
	
		<input class="create_input" type="text" name="firstname" placeholder="Alex">
		<input class="create_input" type="text" name="lastname" placeholder="Palis">


		<input class="button_create button1" name="data" type="submit" value="Verzenden">
</div>
 <a href="<?= URL . 'client/index/'?>">terug gaan</a>
	<style type="text/css">	table{	color: white;	}	th {	background-color: #E64A19; color: white;	}	a:visited {	color: green;	}	a {	color: green;}	a{color: white;}</style>